...
const HowItWorksSection = () => {
  const howItWorksList = [
    {
      ...
      description:
        "Sign up in a few clicks and get access to everything you need, no tech skills required.",
    },
    {
      title: "Build An AI Assistant",
      description:
        "Customize your bot to match your goals. From replies to tone, to remain consistent .",
    },
    {
      title: "Connect Your API",
      description:
        "Link your systems so your bot can send, receive, and act on real-time data",
    },
    {
  ...
  return (
    <section className="w-full flex justify-center">
      <main className="max-w-screen-2xl px-5 md:px-8 lg:px-20 py-20 flex flex-col gap-y-8 lg:gap-y-16">
        ...
        <div className="flex flex-col lg:flex-row gap-y-12 items-center gap-x-16">
          <section>
            <img
              src={sectionImage}
              alt="section image"
          ...
          <section className="flex flex-col gap-y-6">
            {howItWorksList.map((listItem, index) => (
              <div key={index} className="flex gap-x-8 md:gap-x-16">
                <div className="flex flex-col items-center gap-y-2 text-blue-600">
                  <p className="font-bold text-2xl md:text-3xl">0{index + 1}</p>
                  <div className="w-[2px] h-full bg-blue-600" />
                </div>
                <div key={index} className="flex flex-col gap-y-3">
                  <h4 className="text-2xl font-bold">{listItem.title}</h4>
                  <p className="text-lg">{listItem.description}</p>
                </div>
              </div>
            ))}
          </section>
