  ...
  customer_support_contact: Yup.string()
    ...
    .test(
      ...
      (value) => {
        ...
        // Phone validation (allows various formats: +1234567890, 123-456-7890, (123) 456-7890, 09065764742, etc.)
        const phoneRegex = /^[\+]?[0-9]{10,15}$/;
        if (phoneRegex.test(value.replace(/[\-\s\(\)]/g, ""))) return true;

        // Website URL validation (allows http://, https://, or just domain.com)
        const urlRegex =
...
export const companyDetailsSchema = Yup.object({
  ...
  companyUrl: Yup.string().when("filledForm", {
    ...
    then: (schema) =>
      schema.required("Company url is required").min(5).max(100),
...
export const roleAndAudienceSchema = Yup.object({
  ...
  IndustryAndDescription: Yup.string()
    .required()
    .test(
      "wordCount",
      "Max 50 words",
  ...
  welcomeWords: Yup.string()
    .required("Please input tone description")
    .test(
      "wordCount",
      "Max 50 words",
      (val) => !val || val.trim().split(/\s+/).length <= 50
...
export const settingsSchema = Yup.object({
  ...
  additionalInstruction: Yup.string()
    .required("Additional instructions are neccessary")
    .test(
      "wordCount",
      "Max 50 words",
      (val) => !val || val.trim().split(/\s+/).length <= 50
...
export const faqsSchema = Yup.object({
  faqQuestion: Yup.string()
    .required("This questions are important")
    .test(
      "wordCount",
      "Max 50 words",
      (val) => !val || val.trim().split(/\s+/).length <= 50
  ...
  faqAnswer: Yup.string()
    .required("Please provide the answer to this question")
    .test(
      "wordCount",
      "Max 50 words",
      (val) => !val || val.trim().split(/\s+/).length <= 50
  ...
  assistant_id: Yup.number().when("filledForm", {
    ...
    then: (schema) =>
      schema
        ...
        .typeError("Assistant ID must be a number")
        .positive()
        .integer(),
