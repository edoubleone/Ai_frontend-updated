...
const SelectionTab: React.FC<SelectionTabProps> = ({
  ...
  const [isWhenToCollectInformation, setIsWhenToCollectInformation] =
    React.useState<string>(option[0]);
    ...
    <div className={inputStyles + " flex flex-wrap justify-between gap-2 w-full"}>
      {option.map((option, index) => (
        <button
          key={index}
          type="button"
          className={`border capitalize flex-1 text-[14px] w-[calc(100%/3)] rounded-lg px-4 py-2 ${
            isWhenToCollectInformation === option
              ? "bg-blue-600 border-blue-600 text-white"
              : "bg-[#E7E7E7] text-gray-900 hover:bg-blue-400"
          } transition-colors`}
